...
for i in $(seq 1 17); do
    ...
    if [ -d "DScourseS23" ]; then
        points=$((points + 5))
    else
        echo "Directory named 'DScourseS23' does not exist"
    fi
    
    # 5 -- .tex file in PS2 folder
    if ls /home/ouecon${j}/DScourseS23/ProblemSets/PS2/*_*.tex 1> /dev/null 2>&1; then
        points=$((points + 5))
    else
    ...
    if ls /home/ouecon${j}/DScourseS23/ProblemSets/PS2/*_*.pdf 1> /dev/null 2>&1; then
        points=$((points + 5))
    else
